...
*   **Action :** Exécuter la commande suivante à la racine du projet (Prettier est déjà présent dans le `package.json`) :
    ...
    npm install
    ```
    *(Pour ajouter manuellement Prettier dans un nouveau projet : `npm install --save-dev prettier`)*
...
*   **Action :**
    ...
    2. Si des erreurs de formatage sont détectées, appliquez le formatage automatique :
       ...
       npm run format
       ```
    3. *Note importante :* La vérification finale dépend de l'état entièrement formaté du dépôt. Actuellement, tous les fichiers du dépôt respectent ces règles et la commande `npm run format:check` est passante (vert).
