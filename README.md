...
Commit messages follow `<emoji> <type>(<scope>): <message>` (Conventional Commits + emoji). PRs auto-label by area via the `Label` workflow, and titles are auto-normalized via the [my-housekeeper](https://github.com/qveys/my-housekeeper) `normalize-title` handler.
